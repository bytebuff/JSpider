/*********************************************************************************
    ...
    * 懒人听书JS登陆破解 DES加密
    * 在登陆的过程中有个 hashPass 参数 该参数是属于密码加密 在加密的过程中有个token需要传递进去
    * 该token在上一个的请求返回的响应中 响应是一个JSON数据 从该响应中获取即可
    * 第一个请求的网址: http://www.lrts.me/user/login_token.do
    * 第二个请求的网址: http://www.lrts.me/user/login.do
    * 登陆界面: http://www.lrts.me/login
...
/*
    password 是传递的密码 例如: scrapy@qq.com
    token 是需要在上一个变量中获取的值

*/

function hashPass(password) {
	var token = "16c880b946b6c44a";  // token 需要在上个请求中获取的
    ...
    // 转换为字符串
    return encryptedPassword = encryptedPassword.toString();
}